...
md"""
# Computing Model Equilibria 

In this lecture,  we build codes to parameterize the model economy
of Lecture 1, compute an equilibrium, and simulate time series that
can be compared to data.
We will then use the model to revisit the Griliches data woes.
...
model because I abstracted from distortionary taxation.
...
The first is a "brute force" method
