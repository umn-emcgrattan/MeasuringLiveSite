...
> **Note:** Taxable income for the tax on profits is net of depreciation and property tax, and taxable income for the tax on distributions is net of property tax and profits tax.

The aggregate production function is $$Y_{t} = A_{t}F(K_{t}, H_{t}),$$ where capital letters denote aggregates. The parameter $A_{t}$ is TFP that varies over time. The firm rents capital and labor. If profits are maximized, then the rental rates are equal to the marginal products. The goods market clears so $N_{t}(c_{t} + x_{t}) = Y_{t}$. Here, c includes both private and public consumption, and x includes both private
and public investment.

## The Puzzling US Boom in the 1990s

We now show that this basic growth model generates grossly counterfactual predictions during the 1990s. To do so, we compute the model’s equilibrium path with households having perfect foresight of future changes in tax rates, TFP, and populations. 
...
h           = observables[:, 7]
tauc_t      = observables[:, 9]
gtauh       = observables[:, 11]
gtaux       = fill(tauxs, T)
tauk        = fill(tauks, T)
taup        = fill(taups, T)
taud        = fill(tauds, T)
...
kn          = copy(k)
for i in 1:T-1
    kn[i]   = (delt1*k[i] + x[i]) / grate
    k[i+1]  = kn[i]
end
kn[T]       = (delt1*k[T] + x[T]) / grate
lwedge      = psi .* (1 .+ tauc_t) .* c .* h ./ (thet1 .* y .* (1 .- h) .* (1 .- gtauh))
A           = y ./ (k .^ theta .* h .^ thet1)
xwedge      = ones(T)
xtau        = (1 .+ gtaux) .* (1 .- taud) ./ (1 .+ tauc_t)
dtau        = (1 .- taud) .* taup ./ (1 .+ tauc_t)
ptau        = (1 .- taup) .* (1 .- taud) ./ (1 .+ tauc_t)
uc          = c .^ (-sigma) .* (1 .- h) .^ (psi * (1 - sigma))
r           = theta .* y ./ k

# xwedge forwards
for i in 1:T-1
    xwedge[i+1] = (bhat*delt1*uc[i+1]*xtau[i+1]) /
                  (uc[i]*xtau[i]/xwedge[i] -
                   bhat*uc[i+1]*ptau[i+1]*(r[i+1] - tauk[i+1]) -
                   bhat*uc[i+1]*delta*dtau[i+1])
end

inputs      = hcat(A, lwedge, xwedge, g, tauc_t, gtauh, gtaux, tauk, taup, taud)
initial     = hcat(kn, h)
...
begin
function bca_resid(X::AbstractVector{<:Real}, param::AbstractVector{<:Real})
    # parameters
    beta        = float(param[1])
    delta       = float(param[2])
    eta         = float(param[3])
    ...
    inputs_flat = param[10 : 9 + 10*T]
    inputs      = reshape(collect(inputs_flat), T, 10)
    ...
    taup        = inputs[:, 9]
    taud        = inputs[:,10]
    deriv       = float(param[10 + 10*T])
    bhat        = beta * (1 + gamma)^(-sigma)
    thet1       = 1 - theta
    delt1       = 1 - delta
    grate       = (1 + eta) * (1 + gamma)
    tauh        = 1 .- (1 .- gtauh) .* lwedge
    taux        = (1 .+ gtaux) ./ xwedge .- 1

    # variables appearing in residuals
    kn          = collect(float.(X[1:T])) 
    h           = collect(float.(X[T+1:2*T])) 
    k           = vcat(k0, kn[1:T-1]) 
    x           = grate .* kn .- delt1 .* k
    y           = A .* (k .^ theta) .* (h .^ thet1)
    c           = y .- x .- g
    ...
    t           = 1:T-1
    t1          = 2:T
    xtau        = (1 .+ taux) .* (1 .- taud) ./ (1 .+ tauc)
    dtau        = (1 .- taud) .* taup ./ (1 .+ tauc)
    ptau        = (1 .- taup) .* (1 .- taud) ./ (1 .+ tauc)

    euler       = xtau[t] .* uc[t] .- bhat .* uc[t1] .* (ptau[t1] .* (r[t1] .- tauk[t1]) .+ delt1 .* xtau[t1] .+ delta .* dtau[t1])
    terminal    = kn[T] .- deriv .* kn[T-1]
    intrat      = psi .* (1 .+ tauc) .* c .* h ./ y .- thet1 .* (1 .- tauh) .* l

    foc         = vcat(euler, terminal, intrat)
    return foc
end
...
function secant(func::Function, x0::AbstractVector{<:Real}, param::AbstractVector{<:Real};
                crit::Float64=1e-8, maxit::Int=100)

    x0 = collect(float.(x0))
    n  = length(x0)
    # MATLAB: del = diag(max(abs(x0)*1e-4,1e-8))
    stepsz = max.(abs.(x0) .* 1e-4, 1e-8 .* ones(n))
    delmat = Matrix(I, n, n) .* stepsz'  # each column j has stepsz[j] on row j
    ...
    for it in 1:maxit
        f  = func(x, param)
        @assert length(f) == n "Dimension mismatch: f(x) must have length $n."

        # Finite-difference Jacobian
        for j in 1:n
            xpert = copy(x)
            xpert[j] -= stepsz[j]
            fj[:, j] = (f .- func(xpert, param)) ./ stepsz[j]
        end

        # Newton step: x = x - J^{-1} f
        dx = fj \ f
        xnew = x .- dx

        # Convergence: max(|x-x0|/(1+|x0|)) < crit   (note: original compares to previous iterate)
        if maximum(abs.(xnew .- x) ./ (1 .+ abs.(x))) < crit
            x = xnew
            break
        end
        x = xnew
        if it == maxit
            @warn "WARNING: iteration limit of $maxit exceeded for NEWTON"
        end
    end
...
T0            = 1990
param         = [beta; delta; eta; gamma; psi; sigma; theta]
...
taupt         = inputs[:, 9]
taudt         = inputs[:, 10]
tauht         = 1 .- (1 .- gtauht) .* lwedget
tauxt         = (1 .+ gtauxt) ./ xwedget .- 1

V0            = initial
V0            = Array{Float64}(V0)
deriv         = V0[T, 1] / V0[T - 1, 1]

packed        = vcat(param, ks, float(T), vec(inputs), deriv)

res           = bca_resid(vec(V0), identity.(packed))
V             = secant(bca_resid, vec(V0), identity.(packed); crit=1e-8, maxit=100)
V1            = zeros(T, 2)
V1[:]         = V

ktp           = V1[:, 1]
ht            = V1[:, 2]
kt            = vcat(ks, ktp[1:T-1])
xt            = grate .* ktp .- (1 - delta) .* kt
yt            = At .* kt.^theta .* ht.^thet1
ct            = yt .- xt .- gt
lt            = 1 .- ht
uct           = ct.^(-sigma) .* lt.^(psi*(1 - sigma))
ult           = psi .* ct.^(1 - sigma) .* lt.^(psi*(1 - sigma) - 1)
prt           = yt ./ ht
rt            = theta .* yt ./ kt
proft         = rt .* kt .- delta .* kt .- tauks .* kt
val           = (1 - tauds) * (1 + tauxs) .* ktp
dval          = vcat(NaN, val[2:end] .* grate .- val[1:end-1])

# clean years
raw_years     = observables[:, 1]
clean_year(x) = x isa Real ? Int(round(x)) : begin
    s         = strip(String(x))
    s         = replace(s, r"[^\d\-]" => "")
    parse(Int, s)
end
years         = [clean_year(x) for x in raw_years]

ydat          = observables[:, 2] .* gdps
cdat          = observables[:, 4] .* gdps
hdat          = observables[:, 7]
xdat          = observables[:, 5] .* gdps
tauhdat       = observables[:, 11]
kdat          = zeros(T)
kdat1         = zeros(T)
kdat[1]       = ks
for i in 1:T-1
    kdat1[i]  = (delt1 * kdat[i] + xdat[i]) / grate
    kdat[i+1] = kdat1[i]
end
...
default(
    ...
    legendfontsize   = 8,
    guidefont        = 8, # axis label size
    tickfont         = 8,

    # other visual plot settings
    framestyle       = :box, 
...
In Figure 1, we plot the model’s predicted per capita hours of work along with the US actual per capita hours, indexed so that 1990 equals 100. The difference between the series is striking. Actual per capita hours rose 8 percent between 1992 and 1999, whereas the predicted series falls significantly during the same period.
"""
...
In Figure 2, we plot the model’s predicted output along with US real GDP. Both series are adjusted for population and a secular trend of $1.02^{t}$. Although the boom in output was not quite as large as the boom in hours, the model predicts that the
economy should have been depressed. This counterfactual prediction arises from the fact that the tax rates on labor rose during the 1990s and economy-wide TFP was below trend during most of the decade.
"""
